            ...
            <div class="col-lg-12">
                ...
                <div class="col-md-5">
                    ...
                    <div class="form-group standard_combo">
                        <?= lang('weight', 'weight'); ?>
                        <?= form_input('weight', set_value('weight', ($product ? str_replace('.', ',', $product->weight) : '')), 'class="form-control tip" id="weight"'); ?>
                    </div>
                    <div class="form-group all">
                    ...
                    <div class="form-group standard">
                        ...
                        <?= form_input('cost', (isset($_POST['cost']) ? $_POST['cost'] : ($product ? $this->sma->formatNumber($product->cost) : '')), 'class="form-control tip" id="cost" required="required"') ?>
                    </div>
                    <div class="form-group all">
                        <?= lang('product_price', 'price') ?>
                        <?= form_input('price', (isset($_POST['price']) ? $_POST['price'] : ($product ? $this->sma->formatNumber($product->price) : '')), 'class="form-control tip" id="price" required="required"') ?>
                    </div>
    ...
    $(document).ready(function () {
        ...
        function add_product_item(item) {
            ...
            $.each(items, function () {
                ...
                tr_html += '<td><input class="form-control text-center rprice" name="combo_item_price[]" type="text" value="' + formatNumber(this.price) + '" data-id="' + row_no + '" data-item="' + this.id + '" id="combo_item_price_' + row_no + '" onClick="this.select();"></td>';
        ...
        function calculate_price() {
            ...
            var pp = 0;
            $.each(rows, function () {
                var price = $(this).find('.rprice').val().replace(/\./g, "");
                pp += formatDecimal(parseFloat(price)*parseFloat($(this).find('.rquantity').val()));
                pp2 = formatMoney2(pp);
            });
            $('#price').val(pp2);
            return true;
...
// Add by Bayu
// Untuk Setting format rupiah dan titik
var cost = document.getElementById("cost");
cost.addEventListener("keyup", function(e) {
  cost.value = formatRupiah(this.value);
});
...
promo_price.addEventListener("keyup", function(e) {
  promo_price.value = formatRupiah(this.value);
});
